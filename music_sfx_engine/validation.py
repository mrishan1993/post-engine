...
from music_sfx_engine.schemas import TechnicalAudioQA


def probe_audio(uri: str) -> dict[str, Any]:
    path = Path(uri)
    ...
    if meta_path.exists():
        ...
        return {
            ...
            "channels": data.get("channels") or 2,
...
def validate_audio_artifact(
    ...
    *,
    ...
    if not exists:
        return TechnicalAudioQA(ok=False, file_exists=False, notes=["file missing"])

    raw = path.read_bytes()
    readable = len(raw) > 0
    if not readable:
        return TechnicalAudioQA(ok=False, file_exists=True, readable=False, notes=["empty file"])

    probed = probe_audio(uri)
    source = str(probed.get("source") or "none")
    clipping_risk = False
    silence_risk = False
    corruption_risk = False

    if raw.startswith(b"AMP_AUDIO_STUB") or raw.startswith(b"AMP_SFX_STUB"):
    ...
    technical_score = round(
        0.4 * integrity + 0.3 * duration_score + 0.2 * loudness_score + 0.1 * (1.0 if sample_rate_ok else 0.5),
        4,
    )
    ok = exists and readable and duration_ok and sample_rate_ok and not corruption_risk and technical_score >= 0.7
    return TechnicalAudioQA(
        ok=ok,
        file_exists=exists,
        readable=readable,
